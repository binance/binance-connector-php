...
class TradeApi
{
    ...
    public const contentTypes = [
        ...
        'vipLoanFixedRateBorrow' => ['application/x-www-form-urlencoded'],
    ...
    {
        return $this->vipLoanFixedRateBorrowWithHttpInfo($vipLoanFixedRateBorrowRequest);
    }

    /**
     * Operation vipLoanFixedRateBorrowWithHttpInfo.
     *
     * VIP Loan Fixed Rate Borrow (TRADE)
     *
     * @param VipLoanFixedRateBorrowRequest $vipLoanFixedRateBorrowRequest (required)
     *
     * @return ApiResponse<VipLoanFixedRateBorrowResponse>
     *
     * @throws ApiException              on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     */
    public function vipLoanFixedRateBorrowWithHttpInfo($vipLoanFixedRateBorrowRequest): ApiResponse
    {
        $request = $this->vipLoanFixedRateBorrowRequest($vipLoanFixedRateBorrowRequest);
        ...
        try {
            ...
            switch ($statusCode) {
                case 200:
                    return $this->handleResponseWithDataType(
                        '\Binance\Client\VipLoan\Model\VipLoanFixedRateBorrowResponse',
                        $request,
                        $response,
                    );
            }
    ...
    /**
     * Create request for operation 'vipLoanFixedRateBorrow'.
     *
     * @param VipLoanFixedRateBorrowRequest $vipLoanFixedRateBorrowRequest (required)
     *
     * @return Request
     ...
     * @throws \InvalidArgumentException
     */
    public function vipLoanFixedRateBorrowRequest($vipLoanFixedRateBorrowRequest)
    {
        $contentType = self::contentTypes['vipLoanFixedRateBorrow'][0];

        // verify the required parameter 'vipLoanFixedRateBorrowRequest' is set
        if (null === $vipLoanFixedRateBorrowRequest || (is_array($vipLoanFixedRateBorrowRequest) && 0 === count($vipLoanFixedRateBorrowRequest))) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $vipLoanFixedRateBorrowRequest when calling vipLoanFixedRateBorrow'
            );
        }

        $resourcePath = '/sapi/v1/loan/vip/fixed/borrow';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        $getters = $vipLoanFixedRateBorrowRequest::getters();
        $formParams = [];
        foreach ($getters as $property => $getter) {
            $value = $vipLoanFixedRateBorrowRequest->{$getter}();
            if (!empty($value)) {
                $formParams[$property] = $vipLoanFixedRateBorrowRequest->{$getter}();
            }
        }
        ...
        } elseif (isset($vipLoanFixedRateBorrowRequest)) {
            if (false !== stripos($headers['Content-Type'], 'application/json')) {
                // if Content-Type contains "application/json", json_encode the body
                $httpBody = Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($vipLoanFixedRateBorrowRequest));
            } else {
                $httpBody = $vipLoanFixedRateBorrowRequest;
            }
        }
